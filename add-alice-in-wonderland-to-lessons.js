/**
 * Add Alice in Wonderland Digital Books to Lesson Files
 * Integrates parsed chapters into 4th Grade Days 151-180
 */

const fs = require('fs');
const path = require('path');
const { fourthGradeMapping } = require('./alice-in-wonderland-chapter-mapping.js');

// Load the parsed chapters
const chaptersData = JSON.parse(
  fs.readFileSync('./book-data/alice-in-wonderland-chapters.json', 'utf8')
);
...
function createDigitalBookHTML(chapterNumber, vocabWords) {
  ...
  const config = {
    ...
    coverAuthor: "Lewis Carroll",
    coverColor: "#6B4C9A", // Purple/whimsical
  ...
  return `
    ...
    <div id="alice-book"></div>
    <script>
      const aliceConfig = ${JSON.stringify(config, null, 2)};
      window.aliceBook = new DigitalBook('alice-book', aliceConfig);
    </script>
  `;
}
...
function addDigitalBookToLesson(lessonPath, chapterNumber) {
  ...
  console.log(`✅ Added Alice in Wonderland to ${path.basename(lessonPath)} (Chapter ${chapterNumber})`);
  return true;
}
...
function processAllLessons() {
  console.log('🐰 Adding Alice in Wonderland Digital Books to Lessons...\n');

  let successCount = 0;
  let totalCount = 0;

  console.log('📚 Processing 4th Grade (Days 151-180)...');
