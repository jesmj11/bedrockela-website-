/**
 * Add Wizard of Oz Digital Books to Lesson Files
 * Integrates parsed chapters into 4th Grade Days 1-30
 */

const fs = require('fs');
const path = require('path');
const { fourthGradeMapping } = require('./wizard-of-oz-chapter-mapping.js');

// Load the parsed chapters
const chaptersData = JSON.parse(
  fs.readFileSync('./book-data/wizard-of-oz-chapters.json', 'utf8')
);
...
function createDigitalBookHTML(chapterNumber, vocabWords) {
  ...
  const config = {
    ...
    coverColor: "#1B2A4A", // Dark blue/emerald green theme
  ...
  return `
    ...
    <div id="wizard-book"></div>
    <script>
      const wizardConfig = ${JSON.stringify(config, null, 2)};
      window.wizardBook = new DigitalBook('wizard-book', wizardConfig);
    </script>
  `;
}
...
function addDigitalBookToLesson(lessonPath, chapterNumber) {
  ...
  console.log(`✅ Added Wizard of Oz to ${path.basename(lessonPath)} (Chapter ${chapterNumber})`);
  return true;
}
...
function processAllLessons() {
  console.log('🧙 Adding Wizard of Oz Digital Books to Lessons...\n');

  let successCount = 0;
  let totalCount = 0;

  console.log('📚 Processing 4th Grade (Days 1-30)...');
