...
function generateLesson(data) {
  ...
  // Extract words from skill description
  const skillWords = data.skill.match(/\(([^)]+)\)/)?.[1]?.split(', ') || [];
  const sightWordsList = data.sightWords.split(', ');
  const allWords = [...skillWords, ...sightWordsList];
  ...
  pages.push(`<div class="lesson-page-card content-page">
    ...
    <div style="text-align: center; max-width: 600px; margin: 20px auto;">
      ...
      <div style="text-align: left; padding: 20px; background: #fff3e0; border-radius: 10px; margin: 15px 0;">
        ...
        <p style="margin: 10px 0; font-size: 16px;">🔍 <strong>Find</strong> them in your story</p>
      </div>
    </div>
  </div>`);
