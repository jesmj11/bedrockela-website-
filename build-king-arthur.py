...
# Load data
data = json.load(open(f"{BASE}/king-arthur-data.json"))
story_text = open(f"{BASE}/king-arthur-full.txt").read()

# Split into chapters
chapter_splits = re.split(r'\n(?=Chapter \d+)', story_text)
chapters = {}
for chunk in chapter_splits:
    m = re.match(r'Chapter (\d+)\s*\n(.+?)(?=\nChapter \d+|\Z)', chunk, re.DOTALL)
    if m:
        ch_num = int(m.group(1))
        # Get title (first non-empty line after "Chapter X")
        lines = m.group(2).strip().split('\n')
        title = lines[0].strip() if lines else f"Chapter {ch_num}"
...
regular_days = [21,22,23,24, 26,27,28,29, 31,32,33,34, 36,37,38,39]
assessment_days = [25, 30, 35, 40]

chapter_titles = {
    1: "The Sword in the Stone", 2: "The Crowning of a King", 3: "The Gift of Excalibur",
    4: "The Knights of the Round Table", 5: "Sir Lancelot's Arrival", 6: "The Quest of Sir Gawain",
    7: "Merlin's Warning", 8: "Morgan le Fay's Treachery", 9: "Sir Gareth and the Kitchen Knight",
    10: "The Tournament at Camelot", 11: "The Green Knight's Challenge", 12: "The Holy Grail",
    13: "Sir Galahad's Journey", 14: "The Betrayal of Mordred", 15: "The Last Battle",
...
outdir = f"{BASE}/curriculum/grade5/king-arthur"
