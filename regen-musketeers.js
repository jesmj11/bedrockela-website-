...
const vocabText = fs.readFileSync('./book-data/three-musketeers-vocabulary.txt', 'utf8');
const compText = fs.readFileSync('./book-data/three-musketeers-comprehension.txt', 'utf8');
...
console.log('Regenerating Three Musketeers with full content...\n');

for (let day = 31; day <= 60; day++) {
  if (day % 5 === 0) {
    const w = Math.ceil(day/5) + 1;
...
if(typeof initLessonViewer==='function')initLessonViewer(lesson${day}Config);
  ...
  const chIdx = Math.min(Math.floor((day-31)*0.82), 23);
  ...
  fs.writeFileSync(`4th-grade-day-${String(day).padStart(3,'0')}.html`,
`<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>Lesson ${day} - Three Musketeers</title>
<link rel="stylesheet" href="css/lesson-viewer.css?v=1772665488"></head>
<body class="lesson-viewer"><div id="lesson-container"></div>
<script src="js/lesson-viewer.js?v=1772665488"></script>
<script src="https://www.gstatic.com/firebasejs/9.22.0/firebase-app-compat.js"></script>
...
console.log('\n🎉 Three Musketeers complete (Days 31-60)!');
